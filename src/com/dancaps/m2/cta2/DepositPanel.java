...
public class DepositPanel extends JPanel implements ActionListener {
    ...
    public DepositPanel(OutputPanel outputPanel) {
        /* Constructs the object with a specific configuration for the deposit panel */

        // Passes the outputPanel object reference so that the label can be updated by this object
        this.outputPanel = outputPanel;

        // JLabel configuration
        JLabel label = new JLabel("Enter the deposit amount:");
    ...
    public void actionPerformed(ActionEvent e) {
        /* Sends the deposit amount to the output panel to update the balance and redraw the label */

        // Assigns value based on input
        double deposit = (inputField.getValue() == null) ? 0 : ((Number) inputField.getValue()).doubleValue();

        // Sends the deposit amount to the outputPanel object to update the label
        outputPanel.deposit(deposit);
    }
}
